...
**Permisos / corridas desatendidas (JR, 2026-08-14):** esta Mac Mini se usa **solo para tareas automáticas** y JR no está presente para aprobar nada. Por eso `~/.claude/settings.json` lleva `permissions.defaultMode: "bypassPermissions"`. Aplica a **todas** las sesiones de la máquina, no solo a las programadas: nada pide confirmación. Es la contrapartida deliberada de tener corridas desatendidas. Las dos tareas afectadas son `uk-tendencias-virales` (lun/jue 7am) y `uk-artes-semanales` (dom 6pm).
