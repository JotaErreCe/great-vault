---
type: index
date: 2026-05-07
last-touched: 2026-05-07
tags:
  - finanzas
  - index
---

# Finanzas — Hub

Hub de finanzas personales y patrimonio de [[jr]]. Como de 2026-04.

---

## 📂 Subnotas

- [[ingresos]] — fuentes fijas y variables de ingreso
- [[egresos]] — gastos fijos mensuales y categorías
- [[gastos-sms]] — análisis agregado de gastos desde SMS bancarios
- [[propiedades]] — inmuebles en cartera (Z16, San Mateo, Casa Antigua, Z13)
- [[inversiones]] — IBKR (acciones) + cripto
- [[patrimonio]] — Estado Patrimonial consolidado

---

## 📁 Archivos fuente

- `~/Documents/Finanzas Personales.xlsx` — tracker principal
- `~/Dropbox/Felujo de Caja Casa copy.xlsx` — flujo de caja hogar
- Google Drive: Estado Patrimonial, Flujo de Caja Casa, Control de Horas AMC Legal
- `~/Documents/Personal/Hogar/Flujo de Caja - Casa.xlsx`

---

## 🔗 Relacionado

- [[amc-legal]] — fuente principal de honorarios
- [[understanding-kids]] — fuente de ingreso de Mónica
- [[suscripciones]] — gastos fijos detallados

## Relacionado

- [[wiki/index]]
